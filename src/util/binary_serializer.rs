...
use crate::waves_proto::{recipient, Amount as ProtoAmount, Amount, BurnTransactionData, CreateAliasTransactionData, LeaseCancelTransactionData, LeaseTransactionData, Recipient, TransferTransactionData, MassTransferTransactionData};
    ...
    pub fn tx_body_bytes(transaction: &Transaction) -> Result<Vec<u8>> {
        let proto_data = match transaction.data() {
            ...
            TransactionData::MassTransfer(mass_transfer_tx) => {
                let proto_mass_transfer_tx: MassTransferTransactionData = mass_transfer_tx.try_into()?;
